...
### 🛠️ バックエンド連携と技術的挑戦
- **Prisma v7 と Supabase の最新スタック統合:** Prisma v7 の最新仕様である `@prisma/adapter-pg` を用いて Supabase のコネクションプーリング (Pooler) に接続。環境構築やデプロイ時のSSLエラー・キャッシュ問題など多くの壁にぶつかりましたが、設定ファイルや Vercel のビルドスクリプトを最適化することで安定した運用基盤を確立しました。
